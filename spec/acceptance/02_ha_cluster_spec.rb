...
describe 'ha_adguard HA cluster' do
  ...
  context 'primary node configuration' do
    let(:pp) do
      <<-MANIFEST
        class { 'ha_adguard':
          ...
          vrrp_priority      => 150,
          vrrp_router_id     => 51,
          vrrp_auth_pass     => Sensitive('test_password'),
    ...
    describe 'primary keepalived configuration' do
      ...
      it 'has correct virtual_router_id' do
        result = on(primary_host, 'grep "virtual_router_id 51" /etc/keepalived/keepalived.conf')
        expect(result.exit_code).to eq(0)
      end

      it 'has correct priority' do
        result = on(primary_host, 'grep "priority 150" /etc/keepalived/keepalived.conf')
        expect(result.exit_code).to eq(0)
      end
    ...
    describe 'primary health check script' do
      it 'has health check script file' do
        on(primary_host, 'test -f /usr/local/bin/adguard_health_check.sh')
      end

      it 'health check script is executable' do
        on(primary_host, 'test -x /usr/local/bin/adguard_health_check.sh')
      end

      it 'health check script contains pgrep check' do
        result = on(primary_host, 'grep "pgrep.*AdGuardHome" /usr/local/bin/adguard_health_check.sh')
        expect(result.exit_code).to eq(0)
      end

      it 'health check script contains dig check' do
        result = on(primary_host, 'grep "dig @127\\.0\\.0\\.1" /usr/local/bin/adguard_health_check.sh')
        expect(result.exit_code).to eq(0)
      end
    end
  ...
  context 'replica node configuration' do
    ...
    describe 'replica keepalived configuration' do
      ...
      it 'has correct virtual_router_id' do
        result = on(replica_host, 'grep "virtual_router_id 51" /etc/keepalived/keepalived.conf')
        expect(result.exit_code).to eq(0)
      end

      it 'has correct priority' do
        result = on(replica_host, 'grep "priority 100" /etc/keepalived/keepalived.conf')
        expect(result.exit_code).to eq(0)
      end
    ...
    describe 'replica sync configuration' do
      ...
      it 'sync config has correct interval' do
        result = on(replica_host, 'grep "interval: 600" /etc/adguardhome-sync/config.yaml')
        expect(result.exit_code).to eq(0)
      end

      it 'has sync binary' do
        on(replica_host, 'test -f /opt/adguardhome-sync/adguardhome-sync')
      end

      it 'sync binary is executable' do
        on(replica_host, 'test -x /opt/adguardhome-sync/adguardhome-sync')
      end
    end
  ...
  context 'HA cluster functionality' do
    describe 'VRRP priority configuration' do
      it 'primary has higher priority than replica' do
        ...
        expect(primary_priority).to match(/150/)
        expect(replica_priority).to match(/100/)
      end
    end

    describe 'health checks work' do
      it 'primary health check script executes successfully' do
        result = on(primary_host, '/usr/local/bin/adguard_health_check.sh', acceptable_exit_codes: [0, 1])
      ...
      it 'replica health check script executes successfully' do
        result = on(replica_host, '/usr/local/bin/adguard_health_check.sh', acceptable_exit_codes: [0, 1])
        expect(result.exit_code).to be_between(0, 1)
      end
    end
